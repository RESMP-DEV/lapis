...
// Claude agents get lapis's status hooks through --settings. Running the
// generated commands the way Claude Code does moves the agent's status.
void claudeStatusHooks() {
    QTemporaryDir directory;
    require(directory.isValid(), "hook directory");
    // A quote and a space in the path exercise the hook command's quoting.
    const auto base = QDir(directory.path()).filePath(QStringLiteral("it's lapis"));
    require(QDir().mkdir(base, QFile::ReadOwner | QFile::WriteOwner | QFile::ExeOwner),
            "private registry parent");
    const auto canonical = QFileInfo(base).canonicalFilePath();
    const QString claude = uuid();
    const QString other = uuid();
    WorkspaceOptions options;
    options.storagePath = QDir(canonical).filePath(QStringLiteral("workspace.json"));
    auto record = agentRecord(canonical, claude, "general");
    record.insert(QStringLiteral("harness"), QStringLiteral("claude"));
    record.insert(QStringLiteral("statusHooks"), true);
    writeRegistry(
        options.storagePath,
        QJsonObject{
            {"version", 2},
            {"activeCategory", "general"},
            {"categories",
             QJsonArray{QJsonObject{{"id", "general"}, {"name", "General"}, {"selected", other}}}},
            {"agents", QJsonArray{record, agentRecord(canonical, other, "general")}}});
    Workspace workspace(WorkspaceMode::live, options);
    require(workspace.workspaceError().isEmpty(), "load hook registry");
    auto* item = workspace.session(claude);
    require(item && item->statusSource() == lapis::desktop::SessionPreview::StatusSource::hooks,
            "Claude agents use hook status");
    require(workspace.session(other)->statusSource() ==
                lapis::desktop::SessionPreview::StatusSource::observer,
            "Codex keeps its observer");
    const auto endpoint = QDir(canonical).filePath(claude + QStringLiteral(".sock"));
    QFile settings_file(endpoint + QStringLiteral(".hooks.json"));
    require(settings_file.open(QIODevice::ReadOnly), "hook settings written");
    require((settings_file.permissions() & (QFile::ReadGroup | QFile::ReadOther)) == 0,
            "hook settings are private");
    const auto hooks =
        QJsonDocument::fromJson(settings_file.readAll()).object().value("hooks").toObject();
    for (const auto* event :
         {"SessionStart", "UserPromptSubmit", "PreToolUse", "Stop", "Notification"})
        require(hooks.contains(QLatin1String(event)), "each status hook is present");
    // The reconnect attempt starts from the event loop and fails without a
    // service; let it finish, then stand in for a synchronized connection.
    ...
    const auto run = [&](const char* event, const QByteArray& input = {}) {
        const auto command = hooks.value(QLatin1String(event))
                                 .toArray()
                                 .first()
                                 .toObject()
                                 .value("hooks")
                                 ...
                                 .toString();
        QProcess shell;
        shell.start(QStringLiteral("/bin/sh"), {QStringLiteral("-c"), command});
        require(shell.waitForStarted(3000), "hook shell starts");
        shell.write(input);
        shell.closeWriteChannel();
        require(shell.waitForFinished(3000) && shell.exitCode() == 0 &&
                    shell.readAllStandardOutput().isEmpty(),
                "hooks exit 0 and print nothing");
        // Distinct modification times for the poller.
        QThread::msleep(20);
    };
    const auto expect = [&](const char* kind, const char* message) {
        require(waitFor([&] { return item->statusKind() == QLatin1String(kind); }, 3000), message);
    };
    run("SessionStart");
    expect("idle", "session start reads as ready");
    run("UserPromptSubmit");
    expect("working", "a submitted prompt reads as working");
    run("Stop");
    expect("finished", "Stop reads as a finished turn");
    require(item->unseen(), "an unselected Claude agent finishing is marked");
    run("PreToolUse");
    expect("working", "a tool call reads as working");
    run("Notification", R"({"notification_type":"permission_prompt",)"
                        R"("message":"Claude needs your permission to use Bash"})");
    expect("waiting", "a permission prompt reads as needing a response");
    require(item->statusLabel() == QStringLiteral("Needs your response"), "waiting label");
}

// Harnesses without an observer or hooks get an output-timing estimate that
