...
Workspace::Workspace(WorkspaceMode mode, WorkspaceOptions options)
    ...
    hook_poll_.setInterval(400);
    connect(&hook_poll_, &QTimer::timeout, this, &Workspace::pollHookActivity);
...
void Workspace::watch(SessionPreview* item) {
    ...
    if (item->statusSource() == SessionPreview::StatusSource::hooks && !hook_poll_.isActive())
        hook_poll_.start();
...
bool Workspace::createAgent(const QString& directory, const QString& title,
    ...
    try {
        ...
        const bool hooks = harness == QStringLiteral("claude");
        QStringList arguments;
        if (hooks) {
            writeHookSettings(endpoint);
            arguments = {QStringLiteral("--settings"), hookSettingsPath(endpoint)};
        }
        auto launch = session::validate_launch({program, arguments, project, {100, 30}, mode});
        ...
        const Agent agent{active_category_, endpoint, launch, harness, hooks};
...
void Workspace::loadAgents(const QJsonArray& agents) {
    ...
    for (const auto& value : agents) {
        ...
        restoreStatusHooks(agent, object.value(QStringLiteral("statusHooks")).toBool());
...
void SessionPreview::setHookActivity(const QString& kind) {
    if (hook_activity_ == kind)
        return;
    hook_activity_ = kind;
    emit statusChanged();
}
...
// Claude Code status hooks write one word, and for notifications the event
// JSON, to a private file beside the agent's endpoint. They print nothing and
// exit 0, so they change neither the agent's turn nor its approvals.
QString Workspace::hookSettingsPath(const QString& endpoint) {
    return endpoint + QStringLiteral(".hooks.json");
}
QString Workspace::hookActivityPath(const QString& endpoint) {
    return endpoint + QStringLiteral(".activity");
}
void Workspace::writeHookSettings(const QString& endpoint) {
    const auto quote = [](QString value) {
        return QLatin1Char('\'') + value.replace(QLatin1Char('\''), QStringLiteral("'\\''")) +
               QLatin1Char('\'');
    };
    const auto target = quote(hookActivityPath(endpoint));
    const auto store = QStringLiteral(" > %1.$$ && mv -f %1.$$ %1").arg(target);
    const auto record = [&](const char* kind) {
        return QStringLiteral("printf '%1\\n'").arg(QLatin1String(kind)) + store;
    };
    const auto hook = [](const QString& command, const QString& matcher = {}) {
        QJsonObject entry{
            {"hooks", QJsonArray{QJsonObject{{"type", "command"}, {"command", command}}}}};
        if (!matcher.isEmpty())
            entry.insert(QStringLiteral("matcher"), matcher);
        return QJsonArray{entry};
    };
    const QJsonObject settings{
        {"hooks",
         QJsonObject{{"SessionStart", hook(record("idle"))},
                     {"UserPromptSubmit", hook(record("working"))},
                     {"PreToolUse", hook(record("working"), QStringLiteral("*"))},
                     {"Stop", hook(record("finished"))},
                     {"Notification",
                      hook(QStringLiteral("{ printf 'notification\\n'; cat; }") + store)}}}};
    QSaveFile file(hookSettingsPath(endpoint));
    const auto bytes = QJsonDocument(settings).toJson(QJsonDocument::Indented);
    if (!file.open(QIODevice::WriteOnly) ||
        !file.setPermissions(QFile::ReadOwner | QFile::WriteOwner) ||
        file.write(bytes) != bytes.size() || !file.commit())
        throw std::runtime_error("Cannot write Claude status hooks");
}

void Workspace::pollHookActivity() {
    for (const auto& item : sessions_) {
        if (item->statusSource() != SessionPreview::StatusSource::hooks)
            continue;
        const auto path = hookActivityPath(agents_.value(item->sessionId()).endpoint);
        const QFileInfo info(path);
        if (!info.exists() || info.size() > qint64{64} * 1024)
            continue;
        const auto modified = info.lastModified();
        if (hook_seen_.value(item->sessionId()) == modified)
            continue;
        hook_seen_.insert(item->sessionId(), modified);
        QFile file(path);
        if (!file.open(QIODevice::ReadOnly))
            continue;
        const auto event = QString::fromUtf8(file.readLine()).trimmed();
        if (event == QStringLiteral("working") || event == QStringLiteral("idle") ||
            event == QStringLiteral("finished")) {
            item->setHookActivity(event);
            continue;
        }
        if (event != QStringLiteral("notification"))
            continue;
        const auto body = QJsonDocument::fromJson(file.readAll()).object();
        const auto type = body.value(QStringLiteral("notification_type")).toString();
        const auto message = body.value(QStringLiteral("message")).toString();
        if (type == QStringLiteral("permission_prompt") ||
            message.contains(QStringLiteral("permission"), Qt::CaseInsensitive))
            item->setHookActivity(QStringLiteral("waiting"));
        else if (type == QStringLiteral("idle_prompt"))
            item->setHookActivity(QStringLiteral("finished"));
    }
}

// A turn ending, or a new need for a response, marks an unselected agent.
void Workspace::noteStatus(SessionPreview* item) {
    ...
    // Structured requests are marked with their count; hooks report a prompt
    // for a response without one.
    const bool prompted = now == QStringLiteral("waiting") && item->attentionCount() == 0;
    if (finished || prompted)
        item->setUnseen(true);
}
void Workspace::restoreStatusHooks(Agent& agent, bool requested) {
    // The launch must match the one the running service was created with.
    agent.status_hooks = requested && agent.harness == QStringLiteral("claude");
    if (!agent.status_hooks)
        return;
    agent.launch.arguments = {QStringLiteral("--settings"), hookSettingsPath(agent.endpoint)};
    writeHookSettings(agent.endpoint);
}

SessionPreview::StatusSource Workspace::statusSource(const Agent& agent) {
    if (agent.launch.agent == session::AgentMode::codex)
        return SessionPreview::StatusSource::observer;
    return agent.status_hooks ? SessionPreview::StatusSource::hooks
