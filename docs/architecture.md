...
retains its managed observer. The others use the existing direct PTY transport
and have no activity or approval observation capability yet; a connected TUI
does not imply working, finished or approved. No global hooks, model flags or
approval flags are installed. Native Codex and Claude startup and same-process
...
An agent that goes from working to finished or idle, or starts needing a
response, while another agent is selected is marked `unseen`; selecting it
...
- Claude: lapis launches Claude with `--settings <endpoint>.hooks.json`, whose
  SessionStart, UserPromptSubmit, PreToolUse, Stop and Notification hooks write
  one word (and the notification JSON) to `<endpoint>.activity` with an atomic
  rename; they print nothing and exit 0, so turns and approvals are unchanged.
  The desktop polls the file every 400 ms. Permission notifications read as
  needing a response, which must still be answered in Claude's own TUI. The
  registry records `statusHooks` so a restored agent relaunches with identical
  arguments and fingerprint. Probed with Claude Code 2.1.280 (`-p`, Haiku):
  SessionStart, UserPromptSubmit and Stop fired in order.
